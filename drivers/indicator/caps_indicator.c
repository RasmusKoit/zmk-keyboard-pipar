...
static void update_led_state(void) {
    bool led_should_be_on = caps_lock_active || caps_word_active;
    // Invert the logic: 0 = LED on, 1 = LED off
    gpio_pin_set_dt(&caps_led, led_should_be_on ? 0 : 1);
    LOG_DBG("Caps indicator: %s (caps_lock=%d, caps_word=%d)",
            led_should_be_on ? "ON" : "OFF", caps_lock_active, caps_word_active);
}
