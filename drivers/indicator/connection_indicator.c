...
static void update_connection_led(bool usb_connected) {
    // Invert the logic: 0 = LED on (BT), 1 = LED off (USB)
    gpio_pin_set_dt(&connection_led, usb_connected ? 1 : 0);
    LOG_DBG("Connection indicator: %s", usb_connected ? "USB (OFF)" : "BT (ON)");
}
...
static int usb_conn_state_changed_listener(const zmk_event_t *eh) {
    ...
    update_connection_led(ev->conn_state == ZMK_USB_CONN_HID);

    return 0;
}

ZMK_LISTENER(connection_indicator, usb_conn_state_changed_listener);
ZMK_SUBSCRIPTION(connection_indicator, zmk_usb_conn_state_changed);
